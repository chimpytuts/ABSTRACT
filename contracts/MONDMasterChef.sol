...
contract MONDToken is ERC20("MONDToken", "MOND"), Ownable {
    uint256 public constant MAX_SUPPLY = 250_000_000e18; // 250 million mond
...
/*
    This master chef is based on SUSHI's version with some adjustments:
     ...
     - update of emission rate with upper limit of 6 MOND/block
     - more require checks in general
*/
contract MONDMasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of MOND
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMONDPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMONDPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // Info of each pool.
    struct PoolInfo {
        // we have a fixed number of MOND tokens released per block, each pool gets his fraction based on the allocPoint
        uint256 allocPoint; // How many allocation points assigned to this pool. the fraction MOND to distribute per block.
        uint256 lastRewardBlock; // Last block number that MOND distribution occurs.
        uint256 accMONDPerShare; // Accumulated MOND per LP share. this is multiplied by ACC_MOND_PRECISION for more exact results (rounding errors)
    }
    // The MOND TOKEN!
    MONDToken public mond;

    // Treasury address.
    address public treasuryAddress;

    // MOND tokens created per block.
    uint256 public mondPerBlock;

    uint256 private constant ACC_MOND_PRECISION = 1e12;
    ...
    // The block number when MOND mining starts.
    ...
    event LogUpdatePool(
        ...
        uint256 lpSupply,
        uint256 accMONDPerShare
    );
    event SetTreasuryAddress(
        address indexed oldAddress,
        address indexed newAddress
    );
    event UpdateEmissionRate(address indexed user, uint256 _mondPerSec);

    constructor(
        MONDToken _mond,
        address _treasuryAddress,
        uint256 _mondPerBlock,
        uint256 _startBlock
    ) {
        require(
            _mondPerBlock <= 6e18,
            "maximum emission rate of 6 mond per block exceeded"
        );
        mond = _mond;
        treasuryAddress = _treasuryAddress;
        mondPerBlock = _mondPerBlock;
    ...
    // Update the given pool's MOND allocation point. Can only be called by the owner.
    ...
    // View function to see pending MOND on frontend.
    function pendingMOND(uint256 _pid, address _user)
        external
        view
        returns (uint256 pending)
    {
        ...
        // how many MOND per lp token
        uint256 accMONDPerShare = pool.accMONDPerShare;
        ...
        if (block.timestamp > pool.lastRewardBlock && lpSupply != 0) {
            ...
            // based on the pool weight (allocation points) we calculate the mond rewarded for this specific pool
            uint256 mondRewards = (blocksSinceLastReward *
                mondPerBlock *
                pool.allocPoint) / totalAllocPoint;

            // we take parts of the rewards for treasury, these can be subject to change, so we recalculate it
            // a value of 1000 = 100%
            uint256 mondRewardsForPool = (mondRewards * POOL_PERCENTAGE) /
                1000;

            // we calculate the new amount of accumulated mond per LP token
            accMONDPerShare =
                accMONDPerShare +
                ((mondRewardsForPool * ACC_MOND_PRECISION) / lpSupply);
        }
    ...
    function updatePool(uint256 _pid) public returns (PoolInfo memory pool) {
        ...
        if (block.timestamp > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                mond.mint(
                    treasuryAddress,
                    (mondRewards * TREASURY_PERCENTAGE) / 1000
                );

                mond.mint(address(this), mondRewardsForPool);

                pool.accMONDPerShare =
                    pool.accMONDPerShare +
                    ((mondRewardsForPool * ACC_MOND_PRECISION) / lpSupply);
            }
    ...
    // Deposit LP tokens to MasterChef for MOND allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _to
        ...
        // note that only the accMONDPerShare have the precision applied
        user.rewardDebt =
            user.rewardDebt +
            (_amount * pool.accMONDPerShare) /
            ACC_MOND_PRECISION;

        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onMONDReward(_pid, _to, _to, 0, user.amount);
        }
    ...
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _to Receiver of MOND rewards.
    function harvest(uint256 _pid, address _to) public {
        ...
        uint256 accumulatedMOND = (user.amount * pool.accMONDPerShare) /
            ACC_MOND_PRECISION;
        // subtracting the rewards the user is not eligible for
        uint256 eligibleMOND = accumulatedMOND - user.rewardDebt;

        // we set the new rewardDebt to the current accumulated amount of rewards for his amount of LP token
        user.rewardDebt = accumulatedMOND;

        if (eligibleMOND > 0) {
            safeMONDTransfer(_to, eligibleMOND);
        }

        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onMONDReward(
                _pid,
                msg.sender,
                _to,
                eligibleMOND,
        ...
        emit Harvest(msg.sender, _pid, eligibleMOND);
    }

    /// @notice Withdraw LP tokens from MCV and harvest proceeds for transaction sender to `_to`.
    /// @param _pid The index of the pool. See `poolInfo`.
    /// @param _amount LP token amount to withdraw.
    /// @param _to Receiver of the LP tokens and MOND rewards.
    function withdrawAndHarvest(
        ...
        /*
            ...
            => userInfo.amount * pool.accMONDPerShare / ACC_MOND_PRECISION == userInfo.rewardDebt
    ...
    function emergencyWithdraw(uint256 _pid, address _to) public {
        ...
        IRewarder _rewarder = rewarder[_pid];
        if (address(_rewarder) != address(0)) {
            _rewarder.onMONDReward(_pid, msg.sender, _to, 0, 0);
        }
    ...
    // Safe MOND transfer function, just in case if rounding error causes pool to not have enough MOND.
    function safeMONDTransfer(address _to, uint256 _amount) internal {
        uint256 mondBalance = mond.balanceOf(address(this));
        if (_amount > mondBalance) {
            mond.transfer(_to, mondBalance);
        } else {
            mond.transfer(_to, _amount);
        }
    }
    ...
    function updateEmissionRate(uint256 _mondPerBlock) public onlyOwner {
        require(
            _mondPerBlock <= 6e18,
            "maximum emission rate of 6 mond per block exceeded"
        );
        mondPerBlock = _mondPerBlock;
        emit UpdateEmissionRate(msg.sender, _mondPerBlock);
    }
}
